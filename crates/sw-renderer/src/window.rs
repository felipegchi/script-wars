...
impl Window {
    ...
    pub fn center_window(&self) {
        if let Some(monitor) = self.window.current_monitor() {
            ...
            let new_x = (screen_size.width - window_size.width) as f64 / 2.0 + monitor.position().x as f64;
            let new_y = (screen_size.height - window_size.height) as f64 / 2.0 + monitor.position().y as f64;

            // Set the window's outer position to center it on the screen
